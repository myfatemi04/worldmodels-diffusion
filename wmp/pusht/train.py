...
def populate_sample_video_dir():
    ...
    os.makedirs("sample_dataset/videos", exist_ok=True)
    os.makedirs("sample_dataset/metas", exist_ok=True)
...
# export LD_LIBRARY_PATH=/root/cosmos-predict2.5/.venv/lib/python3.10/site-packages/nvidia/cuda_runtime/lib
# torchrun --nproc_per_node=1 --master_port=12341 -m scripts.train --config=cosmos_predict_2_train.py -- experiment=predict2_video2world_training_2b_pusht_128
...
from cosmos_predict2._src.imaginaire.utils.config_helper import (
    import_all_modules_from_package,
)
from cosmos_predict2._src.predict2.configs.common.defaults.checkpoint import (
    register_checkpoint,
)
from cosmos_predict2._src.predict2.configs.common.defaults.ckpt_type import (
    register_ckpt_type,
)
from cosmos_predict2._src.predict2.configs.common.defaults.dataloader import (
    register_training_and_val_data,
)
from cosmos_predict2._src.predict2.configs.common.defaults.ema import register_ema
from cosmos_predict2._src.predict2.configs.common.defaults.optimizer import (
    register_optimizer,
)
from cosmos_predict2._src.predict2.configs.common.defaults.scheduler import (
    register_scheduler,
)
from cosmos_predict2._src.predict2.configs.common.defaults.tokenizer import (
    register_tokenizer,
)
from cosmos_predict2._src.predict2.configs.video2world.defaults.callbacks import (
    register_callbacks,
)
from cosmos_predict2._src.predict2.configs.video2world.defaults.conditioner import (
    register_conditioner,
)
from cosmos_predict2._src.predict2.configs.video2world.defaults.model import (
    register_model,
)
...
# torchrun --nproc_per_node=1 --master_port=12341 -m scripts.train --config=cosmos_predict2/_src/predict2/configs/video2world/config.py -- experiment=predict2_video2world_training_2b_pusht_128
predict2_video2world_training_2b_pusht_128 = dict(
    defaults=[
...
def make_config() -> Config:
    ...
    import_all_modules_from_package(
        "cosmos_predict2._src.predict2.configs.video2world.experiment", reload=True
    )
    try:
        if importlib.util.find_spec("cosmos_predict2.experiments.internal") is not None:
            import_all_modules_from_package(
                "cosmos_predict2.experiments.internal", reload=True
            )
    ...
    for _item in [
        predict2_video2world_training_2b_pusht_128,
    ]:
        # Get the experiment name from the global variable
        experiment_name = [
            name.lower() for name, value in globals().items() if value is _item
        ][0]
        cs.store(
            group="experiment",
            package="_global_",
